...
use concilium_language_engine::corpus::load_glosses_from_data_dir;
...
fn main() {
    ...
    let glosses = load_glosses_from_data_dir(input_dir).expect("failed to load corpus data");
    let gloss_refs = glosses.iter().map(String::as_str).collect::<Vec<_>>();
    let language =
